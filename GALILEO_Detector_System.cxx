...
{
  ifstream file("Configuration_Files/Germanium/GALILEO_Detector_Map.txt");
...
void GALILEO_Detector_System::Process_MBS(int* pdata){
  ...
  while (FEBEX_add->add == 0xadd){
    ...
    if (FEBEXhead->ff == 0xFF){ // FEBEX module idicator //
            ...
            tmp_Sum_Time = (fbx_time->evt_time)+ (tmp_ext_time<<32)*10;//((fbx_hT->ext_time)<<32); In ns
          
         // cout<<" tmp_ext_time " <<tmp_ext_time<< " tmp_Sum_Time " << tmp_Sum_Time<<" tmp_ext_time " << (tmp_ext_time<<32)<<" fbx_time->evt_time " <<fbx_time->evt_time << " (fbx_time->evt_time)+ (tmp_ext_time " <<(fbx_time->evt_time)+ (tmp_ext_time) <<  endl;
      ...
      for(int j = 15; j >= 0; j--)
      {
        ...
        //   cout<<"Chan_Time[fired_FEBEX_amount] " << Chan_Time[fired_FEBEX_amount]<<endl;
           // cout<<" (fbx_Ch_TS->chan_ts) " <<(fbx_Ch_TS->chan_ts) << " Chan_Time[fired_FEBEX_amount] " <<  Chan_Time[fired_FEBEX_amount]<<" tmp_ext_chan_ts " << (tmp_ext_chan_ts<<32)<< endl;
